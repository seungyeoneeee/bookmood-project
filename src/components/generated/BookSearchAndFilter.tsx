    ...
    y: -20
      ...
      <div className="max-w-sm mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <button onClick={() => setCurrentStep('emotion-filter')} className="w-10 h-10 bg-gray-100 rounded-xl flex items-center justify-center hover:bg-gray-200 transition-colors">
        ...
        <div className="bg-gradient-to-r from-[#A8B5E8]/10 to-[#B5D4C8]/10 rounded-2xl p-6 border border-gray-100 mb-6">
          ...
          <button onClick={() => setCurrentStep('emotion-filter')} className="w-full py-3 bg-gradient-to-r from-[#B5D4C8] to-[#A8D4C8] text-white rounded-xl font-medium shadow-md hover:shadow-lg transition-shadow">
            감정별 책 찾기
          </button>
        </div>

        {/* Popular Books Suggestion */}
        <div className="bg-gradient-to-r from-[#F4E4B8]/10 to-[#F0E4B8]/10 rounded-2xl p-6 border border-gray-100">
          <h3 className="text-lg font-semibold text-gray-800 mb-4 flex items-center">
      ...
      x: -20
        ...
        <div className="max-w-sm mx-auto">
          ...
          <div className="space-y-6 mb-8">
            {filterEmotionCategories.map(category => <div key={category.category} className="bg-white border border-gray-200 rounded-2xl p-6 shadow-sm">
                <h3 className="text-lg font-semibold text-gray-800 mb-4">
                  {category.category}
                </h3>
                <div className="grid grid-cols-2 gap-3">
                  {category.emotions.map(emotion => <button key={emotion.name} onClick={() => toggleFilterEmotion(emotion.name)} className={`p-3 rounded-xl border-2 transition-all text-left ${selectedFilterEmotions.includes(emotion.name) ? 'border-[#A8B5E8] bg-[#A8B5E8]/10' : 'border-gray-200 hover:border-gray-300'}`}>
          ...
          {/* Selected Emotions */}
          {selectedFilterEmotions.length > 0 && <div className="bg-[#A8B5E8]/10 border border-[#A8B5E8]/20 rounded-2xl p-4 mb-6">
              <div className="flex items-center justify-between mb-3">
                <h4 className="font-medium text-gray-800">선택된 감정</h4>
                <button onClick={() => setSelectedFilterEmotions([])} className="text-xs text-gray-600 hover:text-gray-800">
                  모두 해제
                </button>
              </div>
              ...
              return <span key={emotion} className="px-3 py-1 bg-white rounded-full text-sm font-medium text-gray-700 border border-gray-200 flex items-center space-x-2">
                  ...
                  backgroundColor: emotionData?.color || '#A8B5E8'
                      ...
                      <span>{emotion}</span>
                      <button onClick={() => toggleFilterEmotion(emotion)} className="text-gray-400 hover:text-gray-600">
                        <X className="w-3 h-3" />
                      </button>
                    </span>;
            })}
                    ...
                    <div className="flex-1 space-y-2">
                      ...
                      <div className="flex items-center justify-between pt-2">
                        <div className="flex space-x-2">
                          <button onClick={() => handleBookSelect(book)} className="px-3 py-1 text-xs bg-[#A8B5E8] text-white rounded-lg hover:bg-[#8BB5E8] transition-colors">
                            읽기
                          </button>
                        ...
                        <button onClick={() => onWishlistToggle?.(book)} className="p-1 text-gray-400 hover:text-pink-500 transition-colors">
                          <Heart className={`w-4 h-4 ${wishlistBooks.includes(book.id) ? 'fill-current text-pink-500' : ''}`} />
                        </button>
                      </div>
                    </div>
                  </div>
          ...
          opacity: 1
            ...
            setMinRating(0);
            setSortType('relevance');
          }} className="px-4 py-2 bg-[#A8B5E8] text-white rounded-xl text-sm hover:bg-[#8BB5E8] transition-colors">
                필터 초기화
              </button>
            </motion.div>}
        </div>
